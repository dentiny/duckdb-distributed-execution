...
TEST_CASE("Worker result uses coordinator logical metadata", "[worker][types]") {
	...
	auto worker_result = make_uniq<MaterializedQueryResult>(StatementType::SELECT_STATEMENT, StatementProperties {},
	                                                        vector<string> {"worker_value"}, std::move(collection),
	                                                        ClientProperties {});
	...
	REQUIRE(result->types == coordinator_types);
	REQUIRE(result->names[0] == coordinator_names[0]);
	REQUIRE(materialized.Collection().Types() == coordinator_types);
