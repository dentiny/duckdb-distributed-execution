...
One build command
```sh
export VCPKG_TOOLCHAIN_PATH=/home/vscode/vcpkg/scripts/buildsystems/vcpkg.cmake && OVERRIDE_GIT_DESCRIBE=v1.4.1 CMAKE_BUILD_PARALLEL_LEVEL=$(nproc) make debug
```
...
Now we can use the features from the extension directly in DuckDB. The template contains a single scalar function `duckherder()` that takes a string arguments and returns a string:
```
D select duckherder('Jane') as result;
┌───────────────┐
│    result     │
│    varchar    │
├───────────────┤
